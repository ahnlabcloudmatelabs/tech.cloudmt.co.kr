...
친구가 액티비티펍 서버를 하나 팠다는데, 저한테 관리를 좀 해줄 수 있냐고 물어봐서  
이게 쉽지 않을텐데 혼자 했다길래 걱정 반으로 어찌 했나 봐주겠다고 하며 시작됐습니다.  
도메인이 [yodangang.express](https://yodangang.express) 라서 이녀석, 본격적으로 하려나본데 하면서 말이죠.

[Misskey] ~~링크 클릭 전에 10덕 주의~~ 라는걸로 구축했던데,  
뭐…. 으에에ㅔ 이 10덕은 뭐야 하면서 구웨에에엑 하다가

![](images/1.webp)

VM 들어가보니 친구가 생각보다 잘 해놨길래  
"어… 야, 개발자 함 해볼래?"  
하고 농담하면서 직접 관리해주기로 하며 시작됐습니다.
...
다들 자기가 많이 한 개발 분야가 달라서 숙달된 정도도 다르기 땜에  
패시브 스킬(?)로 발동되는 영역이 다를겁니다.  
...
첨엔 서버리스로 올리면 사용자가 적을 땐 돈이 많이 절약되겠지?  
라고 희망회로를 돌리며 컨테이너라이즈 해서 구글의 Cloud Run으로 올렸습니다.
...
내 서버 사용자가 적어도 다른 서버에서 뭔가를 오질나게 보내옵니다.  
(새벽에도 계속 보내옵니다)
...
물리장비에 서버를 세팅할 때는 주의해야 할 점이 몇 가지가 있는데  
(매우 개인적인 관점입니다)
...
다른 컨테이너에 마운트 해서 DB를 복구할 수 있도록 해놨습니다.
